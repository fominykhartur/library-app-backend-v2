...
export function addNewBook(req: Request, res: Response) {
    ...
    pool.getConnection(async (err, connection) => {
        ...
        try {
            ...
            connection.execute(
                "SELECT book_id INTO @bookID FROM books WHERE book_name = ? and author_name = ?;",
                [bookData.bookName, bookData.authorName]
            );
            connection.execute(
                "INSERT IGNORE INTO book_status (user_id,book_id,status) VALUES (?,@bookID,?);",
            ...
            return res
                .status(400)
                .send(JSON.stringify({ status: "New book add failed" }));
        }
    });
}
